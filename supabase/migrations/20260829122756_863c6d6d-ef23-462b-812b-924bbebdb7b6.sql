...
AS $function$
  ...
  loja AS (
    SELECT v.vendedor_id,
      ...
      CASE WHEN v.status = 'instalado'
            AND v.data_ativacao IS NOT NULL
            AND date_trunc('month', v.data_ativacao) = date_trunc('month', _mes_ref)
            AND v.tecnologia ~* 'banda\s*larga|fibra|fttx|internet'
            AND NOT COALESCE(v.classe_protocolo,'') LIKE 'Renovação%' THEN 1 ELSE 0 END AS bl,
      CASE WHEN v.status = 'instalado'
            AND v.data_ativacao IS NOT NULL
            AND date_trunc('month', v.data_ativacao) = date_trunc('month', _mes_ref)
            AND (v.tecnologia ~* 'm[óo]vel|celular|5g|4g' OR v.contem_movel OR COALESCE(v.qtd_linhas,0) > 0)
  ...
  pap AS (
    SELECT v.vendedor_id,
      ...
      CASE WHEN v.status = 'instalado'
            AND v.data_ativacao IS NOT NULL
            AND date_trunc('month', v.data_ativacao) = date_trunc('month', _mes_ref)
            AND (COALESCE(v.produto,'') || ' ' || COALESCE(v.tecnologia,'')) ~* 'banda\s*larga|fibra|fttx|internet'
            AND NOT COALESCE(v.tipo_protocolo,'') LIKE 'Renovação%' THEN 1 ELSE 0 END AS bl,
      CASE WHEN v.status = 'instalado'
            AND v.data_ativacao IS NOT NULL
            AND date_trunc('month', v.data_ativacao) = date_trunc('month', _mes_ref)
            AND ((COALESCE(v.produto,'') || ' ' || COALESCE(v.tecnologia,'')) ~* 'm[óo]vel|celular|5g|4g' OR COALESCE(v.qtd_linhas,0) > 0)
            THEN COALESCE(v.qtd_linhas,0) ELSE 0 END AS mv,
